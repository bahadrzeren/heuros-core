package org.heuros.director;

import java.util.List;

import org.heuros.core.director.AbstractDirector;

/**
 * AbstractDirector implementation written to be used for simple data format transformation purposes.
 * 
 * @author bahadrzeren
 *
 * @param <I> Type of the model that will be subject to format change.
 */
public class DataTransformDirector<I> extends AbstractDirector<I, I> {

	@Override
	public void proceed() {
		if (this.check()) {
			List<I> inputData = this.loader.extractData();
			if (inputData != null)
				this.reporter.reportData(inputData);
		}
	}
}
