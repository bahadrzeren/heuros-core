...
public class PairingGenerator {
	...
	private void generatePartialNetwork(Duty[] duties,
										int maxPairingLengthInDays,
										int[] numOfCoveringsInDuties,
										int[] blockTimeOfCoveringsInDuties,
										boolean[] isDutyInPartialNet,
										boolean mandHbDep,
										boolean mandHbArr) {
	}
	...
	public Pair generatePairing(Leg legToCover, int heuristicNo, int maxPairingLengthInDays,
		...
		if ((duties != null)
			...
			for (Duty duty: duties) {
				...
				boolean[] isDutyInPartialNet = new boolean[numOfCoveringsInDuties.length];

				LegView fl = duty.getFirstLeg();
				LegView ll = duty.getLastLeg();
				if (duty.isHbDep(hbNdx)) {
					if (duty.isHbArr(hbNdx)) {
						//	
					} else {
